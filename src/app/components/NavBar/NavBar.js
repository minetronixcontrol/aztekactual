import React from 'react';
import {Redirect} from 'react-router-dom';
import {Collapse, Navbar, NavbarToggler, NavbarBrand, Nav, NavItem, NavLink, UncontrolledDropdown, DropdownToggle, DropdownMenu, DropdownItem } from 'reactstrap';
//import LogoNavbar from "./img/logo-navbar.png";

const LogoNavbar = 'data:image/png;base64,';

var username = '';
var urlActual = '';

class NavBarMain extends React.Component {

  constructor(props) {
    super(props);

    const { location, match } = props;

    console.log('match', match.url);
    //console.log('location', location);

    urlActual = match.url.split("/", 2);

    username = match.params.id_Usuario;

    this.toggle = this.toggle.bind(this);
    this.redireccionarIrNuevaVenta = this.redireccionarIrNuevaVenta.bind(this);
    this.redireccionarIrModificarVenta = this.redireccionarIrModificarVenta.bind(this);
    this.redireccionarIrVentasConcluidas = this.redireccionarIrVentasConcluidas.bind(this);
    this.redireccionarIrVentasCanceladas = this.redireccionarIrVentasCanceladas.bind(this);
    this.redireccionarIrNuevaReservacion = this.redireccionarIrNuevaReservacion.bind(this);
    this.redireccionarIrModificarReservacion = this.redireccionarIrModificarReservacion.bind(this);
    this.redireccionarIrReservacionesConcluidas = this.redireccionarIrReservacionesConcluidas.bind(this);
    this.redireccionarIrReservacionesCanceladas = this.redireccionarIrReservacionesCanceladas.bind(this);
    this.redireccionarIrRutas = this.redireccionarIrRutas.bind(this);
    this.redireccionarIrClientes= this.redireccionarIrClientes.bind(this);
    this.redireccionarIrOrigenesyDestinos = this.redireccionarIrOrigenesyDestinos.bind(this);
    this.redireccionarIrCamiones = this.redireccionarIrCamiones.bind(this);
    this.redireccionarIrTripulantes = this.redireccionarIrTripulantes.bind(this);
    this.redireccionarIrViajes= this.redireccionarIrViajes.bind(this);
    this.redireccionarIrListaAbordaje = this.redireccionarIrListaAbordaje.bind(this);
    this.redireccionarIrListaNegra = this.redireccionarIrListaNegra.bind(this);
    this.redireccionarReasignacionesPendientes = this.redireccionarReasignacionesPendientes.bind(this);
    this.redireccionarIrCorte= this.redireccionarIrCorte.bind(this);
    this.redireccionarIrCorteNuevo= this.redireccionarIrCorteNuevo.bind(this);
    this.redireccionarIrConfigurarViajes = this.redireccionarIrConfigurarViajes.bind(this);
    this.redireccionarIrUsuarios = this.redireccionarIrUsuarios.bind(this);
    this.redireccionarIrConfiguracionDePublicidad = this.redireccionarIrConfiguracionDePublicidad.bind(this);
    this.redireccionarIrPublicidad = this.redireccionarIrPublicidad.bind(this);
    this.redireccionarIrPapeleraDeReciclaje= this.redireccionarIrPapeleraDeReciclaje.bind(this);
    this.redireccionarIrConfiguracionBoletos = this.redireccionarIrConfiguracionBoletos.bind(this);
    this.redireccionarIrLogin = this.redireccionarIrLogin.bind(this);
    this.redireccionarIrSucursales = this.redireccionarIrSucursales.bind(this);
    this.redireccionarIrDescuentos = this.redireccionarIrDescuentos.bind(this);
    this.redireccionarIrConfigurarSucursales = this.redireccionarIrConfigurarSucursales.bind(this);
    this.redireccionarIrPrecios = this.redireccionarIrPrecios.bind(this);
    this.redireccionarIrConfigurarPrecios = this.redireccionarIrConfigurarPrecios.bind(this);
    this.redireccionarIrViajeRedondo = this.redireccionarIrViajeRedondo.bind(this);
    this.fetchUser = this.fetchUser.bind(this);
    
    this.state = {isOpen: false, IrNuevaVenta:false, IrLogin:false, IrModificarVenta:false, IrVentasConcluidas:false, IrVentasCanceladas:false, IrNuevaReservacion:false, IrModificarReservacion:false, IrReservacionesConcluidas:false, IrReservacionesCanceladas:false, IrRutas:false, IrClientes:false, IrOrigenesyDestinos:false, IrCamiones:false, IrTripulantes:false, IrViajes:false, IrListaAbordaje:false, IrListaNegra:false, ReasignacionesPendientes:false, IrCorte:false,IrConfigurarViajes:false,IrUsuarios:false,IrConfiguracionBoletos:false,IrConfiguracionDePublicidad:false,IrPublicidad:false,IrPapeleraDeReciclaje:false,IrCorteNuevo:false,IrSucursales:false, IrConfigurarSucursales:false,IrPrecios:false, IrConfigurarPrecios:false,IrDescuentos:false, nombre: '', apellidoPaterno: '', apellidoMaterno: '', urlActual: urlActual[1]}
  
  };

    componentDidMount(){
      this.fetchUser();
    }

    fetchUser(){
      fetch(`/api/Usuarios/${username}`)
        .then(res => res.json())
        .then(data => {
            console.log(data);
            this.setState({
              nombre: data[0].nombre,
              apellidoPaterno: data[0].apellidoPaterno,
              apellidoMaterno: data[0].apellidoMaterno
            })
        });
    }

    redireccionarIrNuevaVenta(e){ e.preventDefault(); this.setState({IrNuevaVenta:true})};
    redireccionarIrModificarVenta(e){ e.preventDefault(); this.setState({IrModificarVenta:true})};
    redireccionarIrVentasConcluidas(e){ e.preventDefault(); this.setState({IrVentasConcluidas:true})};
    redireccionarIrVentasCanceladas(e){ e.preventDefault(); this.setState({IrVentasCanceladas:true})};
    redireccionarIrNuevaReservacion(e){ e.preventDefault(); this.setState({IrNuevaReservacion:true})};
    redireccionarIrModificarReservacion(e){ e.preventDefault(); this.setState({IrModificarReservacion:true})};
    redireccionarIrReservacionesConcluidas(e){ e.preventDefault(); this.setState({IrReservacionesConcluidas:true})};
    redireccionarIrReservacionesCanceladas(e){ e.preventDefault(); this.setState({IrReservacionesCanceladas:true})};
    redireccionarIrRutas(e){e.preventDefault(); this.setState({IrRutas:true})};
    redireccionarIrClientes(e){ e.preventDefault(); this.setState({IrClientes:true})};
    redireccionarIrOrigenesyDestinos(e){ e.preventDefault(); this.setState({IrOrigenesyDestinos:true})};
    redireccionarIrDestinos(e){ e.preventDefault(); this.setState({IrDestinos:true})};
    redireccionarIrCamiones(e){ e.preventDefault(); this.setState({IrCamiones:true})};
    redireccionarIrTripulantes(e){ e.preventDefault(); this.setState({IrTripulantes:true})};
    redireccionarIrViajes(e){ e.preventDefault(); this.setState({IrViajes:true})};
    redireccionarIrListaAbordaje(e){ e.preventDefault(); this.setState({IrListaAbordaje:true})};
    redireccionarIrListaNegra(e){ e.preventDefault(); this.setState({IrListaNegra:true})};
    redireccionarReasignacionesPendientes(e){ e.preventDefault(); this.setState({ReasignacionesPendientes:true})};
    redireccionarIrCorte(e){ e.preventDefault(); this.setState({IrCorte:true})};
    redireccionarIrCorteNuevo(e){ e.preventDefault(); this.setState({IrCorteNuevo:true})};
    redireccionarIrConfigurarViajes(e){ e.preventDefault(); this.setState({IrConfigurarViajes:true})};
    redireccionarIrUsuarios(e){ e.preventDefault(); this.setState({IrUsuarios:true})};
    redireccionarIrConfiguracionDePublicidad(e){ e.preventDefault(); this.setState({IrConfiguracionDePublicidad:true})};
    redireccionarIrPublicidad(e){ e.preventDefault(); this.setState({IrPublicidad:true})};
    redireccionarIrPapeleraDeReciclaje(e){ e.preventDefault(); this.setState({IrPapeleraDeReciclaje:true})};
    redireccionarIrLogin(e){ e.preventDefault(); this.setState({IrLogin:true})};  
    redireccionarIrConfiguracionBoletos(e){ e.preventDefault(); this.setState({IrConfiguracionBoletos:true})};
    redireccionarIrSucursales(e){ e.preventDefault(); this.setState({IrSucursales:true})};  
    redireccionarIrDescuentos(e){ e.preventDefault(); this.setState({IrDescuentos:true})};
    redireccionarIrConfigurarSucursales(e){ e.preventDefault(); this.setState({IrConfigurarSucursales:true})};
    redireccionarIrPrecios(e){ e.preventDefault(); this.setState({IrPrecios:true})};
    redireccionarIrConfigurarPrecios(e){ e.preventDefault(); this.setState({IrConfigurarPrecios:true})};    
    redireccionarIrViajeRedondo(e){ e.preventDefault(); this.setState({IrViajeRedondo:true})};
    toggle() {this.setState({isOpen: !this.state.isOpen})};

  render() {
    //Tenemos que reemplazar :id_Usuario por la variable username
    if(this.state.IrCorte){return <Redirect to= {"/Corte/"+username}/>};
    if(this.state.IrCorteNuevo){return <Redirect to= {"/CorteNuevo/"+username}/>};
    if(this.state.IrConfigurarViajes){return <Redirect to= {"/ConfigurarViajes/"+username}/>};
    if(this.state.IrConfiguracionBoletos){return <Redirect to= {"/ConfiguracionBoletos/"+username}/>};
    if(this.state.IrConfiguracionDePublicidad){return <Redirect to= {"/ConfiguracionDePublicidad/"+username}/>};
    if(this.state.IrCamiones){return <Redirect to= {"/Camiones/"+username}/>};//
    if(this.state.IrClientes){return <Redirect to= {"/Clientes/"+username}/>};//
    if(this.state.IrNuevaVenta){return <Redirect to= {"/Index/"+username}/>};//
    if(this.state.IrLogin){return <Redirect to= {"/"+username}/>};
    if(this.state.IrListaAbordaje){return <Redirect to= {"/ListaAbordaje/"+username}/>};
    if(this.state.IrListaNegra){return <Redirect to= {"/ListaNegra/"+username}/>};
    if(this.state.ReasignacionesPendientes){return <Redirect to= {"/ReasignacionesPendientes/"+username+"/"}/>};
    if(this.state.IrNuevaReservacion){return <Redirect to= {"/NuevaReservacion/"+username}/>};//
    if(this.state.IrModificarVenta){return <Redirect to= {"/ModificarVenta/"+username}/>};
    if(this.state.IrModificarReservacion){return <Redirect to= {"/ModificarReservacion/"+username}/>};
    if(this.state.IrOrigenesyDestinos){return <Redirect to= {"/OrigenesyDestinos/"+username}/>};//
    if(this.state.IrPublicidad){return <Redirect to= {"/Publicidad/"+username}/>};
    if(this.state.IrPapeleraDeReciclaje){return <Redirect to= {"/PapeleraDeReciclaje/"+username}/>};
    if(this.state.IrReservacionesConcluidas){return <Redirect to= {"/ReservacionesConcluidas/"+username}/>};
    if(this.state.IrReservacionesCanceladas){return <Redirect to= {"/ReservacionesCanceladas/"+username}/>};
    if(this.state.IrRutas){return <Redirect to= {"/Rutas/"+username}/>};
    if(this.state.IrTripulantes){return <Redirect to= {"/Tripulantes/"+username}/>};
    if(this.state.IrUsuarios){return <Redirect to= {"/Usuarios/"+username}/>};
    if(this.state.IrViajes){return <Redirect to= {"/Viajes/"+username}/>};
    if(this.state.IrVentasConcluidas){return <Redirect to= {"/VentasConcluidas/"+username}/>};
    if(this.state.IrVentasCanceladas){return <Redirect to= {"/VentasCanceladas/"+username}/>};
    if(this.state.IrSucursales){return <Redirect to= {"/Sucursales/"+username}/>};
    if(this.state.IrConfigurarSucursales){return <Redirect to= {"/ConfigurarSucursales/"+username}/>};
    if(this.state.IrDescuentos){return <Redirect to= {"/Descuentos/"+username}/>};
    if(this.state.IrConfigurarPrecios){return <Redirect to= {"/ConfigurarPrecios/"+username}/>};
    if(this.state.IrPrecios){return <Redirect to= {"/Precios/"+username}/>};
    if(this.state.IrViajeRedondo){return <Redirect to= {"/ViajeRedondo/"+username}/>};
   
    return (
      <div>
        <Navbar color="danger" dark expand="md" >
          <NavbarBrand>
            <img src={LogoNavbar} width="auto" height="60" alt="logo-viajes_azteca"/>
          </NavbarBrand>
          <NavbarToggler onClick={this.toggle} />
          <span class="navbar-text" style={{color: 'white'}}>
            {this.state.nombre+" "+this.state.apellidoPaterno+" "+this.state.apellidoMaterno}
          </span>
          <Collapse isOpen={this.state.isOpen} navbar>
            <Nav className="ml-auto" navbar>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  VENTAS
                </DropdownToggle>
                <DropdownMenu left>
                  {(this.state.urlActual == 'Index')? 
                  ''
                  : 
                  (
                    <DropdownItem>
                      <NavItem onClick={this.redireccionarIrNuevaVenta}>
                          <NavLink className="text-dark"> NUEVA VENTA </NavLink>
                      </NavItem>
                    </DropdownItem>
                  )}
                  {(this.state.urlActual == 'ViajeRedondo')? 
                  ''
                  : 
                  (
                    <DropdownItem>
                      <NavItem onClick={this.redireccionarIrViajeRedondo}>
                          <NavLink className="text-dark"> NUEVA VENTA VIAJE REDONDO </NavLink>
                      </NavItem>
                    </DropdownItem>
                  )}
                  {(this.state.urlActual == 'ModificarVenta')? 
                    ''
                    :
                    (
                      <DropdownItem>
                        <NavItem onClick={this.redireccionarIrModificarVenta}>
                            <NavLink className="text-dark"> MODIFICAR VENTA </NavLink>
                        </NavItem>
                      </DropdownItem>
                    )
                  }

                  {(this.state.urlActual == 'VentasConcluidas')? 
                    ''
                    :
                    (
                      <DropdownItem>
                        <NavItem onClick={this.redireccionarIrVentasConcluidas}>
                            <NavLink className="text-dark"> VENTAS CONCLUIDAS </NavLink>
                        </NavItem>
                      </DropdownItem>
                    )
                  }
                  
                  {(this.state.urlActual == 'VentasCanceladas')? 
                  ''
                  :
                  (
                      <DropdownItem>
                        <NavItem onClick={this.redireccionarIrVentasCanceladas}>
                            <NavLink className="text-dark"> VENTAS CANCELADAS </NavLink>
                        </NavItem>
                      </DropdownItem>
                  )
                  }
                  {/*(this.state.urlActual == 'PapeleraDeReciclaje')? 
                  ''
                  :
                  (
                    <DropdownItem>
                      <NavItem onClick={this.redireccionarIrPapeleraDeReciclaje}>
                          <NavLink className="text-dark"> VENTA MULTIPLE </NavLink>
                      </NavItem>
                    </DropdownItem>
                  )
                  */}

                </DropdownMenu>
              </UncontrolledDropdown>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  RESERVACIONES
                </DropdownToggle>
                <DropdownMenu left>
                {(this.state.urlActual == 'NuevaReservacion')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrNuevaReservacion}>
                        <NavLink className="text-dark"> NUEVA RESERVACION </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ModificarReservacion')?
                ''
                :
                ( 
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrModificarReservacion}>
                        <NavLink className="text-dark"> MODIFICAR RESERVACION </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }     
                {(this.state.urlActual == 'ReservacionesConcluidas')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrReservacionesConcluidas}>
                        <NavLink className="text-dark"> RESERVACIONES CONCLUIDAS </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ReservacionesCanceladas')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrReservacionesCanceladas}>
                        <NavLink className="text-dark"> RESERVACIONES CANCELADAS </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }

                </DropdownMenu>
              </UncontrolledDropdown>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  VER
                </DropdownToggle>
                <DropdownMenu left>

                {(this.state.urlActual == 'Clientes')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrClientes}>
                        <NavLink className="text-dark"> CLIENTES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'OrigenesyDestinos')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrOrigenesyDestinos}>
                        <NavLink className="text-dark"> ORIGENES Y DESTINOS </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {/* (this.state.urlActual == 'Precios')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrPrecios}>
                        <NavLink className="text-dark"> PRECIOS </NavLink>
                    </NavItem>
                  </DropdownItem>
                ) */
                }
                {(this.state.urlActual == 'Camiones')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrCamiones}>
                        <NavLink className="text-dark"> CAMIONES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Tripulantes')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrTripulantes}>
                        <NavLink className="text-dark"> TRIPULANTES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Viajes')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrViajes}>
                        <NavLink className="text-dark"> VIAJES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Sucursales')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrSucursales}>
                        <NavLink className="text-dark"> SUCURSALES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }

                </DropdownMenu>
              </UncontrolledDropdown>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  LISTAS
                </DropdownToggle>
                <DropdownMenu right>

                {(this.state.urlActual == 'ListaAbordaje')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem  onClick={this.redireccionarIrListaAbordaje}>
                        <NavLink className="text-dark">LISTA DE ABORDAJE</NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ListaNegra')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrListaNegra}>
                        <NavLink className="text-dark"> LISTA NEGRA </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ReasignacionesPendientes')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarReasignacionesPendientes}>
                        <NavLink className="text-dark"> LISTA REASIGNACIONES PENDIENTES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }

                </DropdownMenu>
              </UncontrolledDropdown>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  CORTE
                </DropdownToggle>
                <DropdownMenu>
                {(this.state.urlActual == 'CorteNuevo')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrCorteNuevo}>
                        <NavLink className="text-dark"> CORTE </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Corte')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrCorte}>
                        <NavLink className="text-dark"> CORTE ANTERIOR</NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                </DropdownMenu>
              </UncontrolledDropdown>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  VIAJES
                </DropdownToggle>
                <DropdownMenu left>
                {(this.state.urlActual == 'ConfigurarViajes')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrConfigurarViajes}>
                        <NavLink className="text-dark"> CONFIGURAR VIAJES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Rutas')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrRutas}>
                        <NavLink className="text-dark"> RUTAS </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                </DropdownMenu>
              </UncontrolledDropdown>
              <UncontrolledDropdown nav inNavbar>
                <DropdownToggle nav caret>
                  OPCIONES
                </DropdownToggle>
                <DropdownMenu right>

                {(this.state.urlActual == 'Usuarios')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem  onClick={this.redireccionarIrUsuarios}>
                        <NavLink className="text-dark">USUARIOS</NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ConfiguracionBoletos')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrConfiguracionBoletos}>
                        <NavLink className="text-dark"> CONFIGURACION DE BOLETOS </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ConfiguracionDePublicidad')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrConfiguracionDePublicidad}>
                        <NavLink className="text-dark"> CONFIGURACION DE PUBLICIDAD </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Publicidad')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrPublicidad}>
                        <NavLink className="text-dark"> PUBLICIDAD </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'Descuentos')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrDescuentos}>
                        <NavLink className="text-dark"> DESCUENTOS </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ConfigurarSucursales')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrConfigurarSucursales}>
                        <NavLink className="text-dark"> SUCURSALES </NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                {(this.state.urlActual == 'ConfigurarPrecios')? 
                ''
                :
                (
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrConfigurarPrecios}>
                        <NavLink className="text-dark"> CONFIGURAR PRECIOS</NavLink>
                    </NavItem>
                  </DropdownItem>
                )
                }
                  <DropdownItem>
                    <NavItem onClick={this.redireccionarIrLogin}>
                        <NavLink className="text-dark"> CERRAR SESION </NavLink>
                    </NavItem>
                  </DropdownItem>
                </DropdownMenu>
              </UncontrolledDropdown>
            </Nav>
          </Collapse>
        </Navbar>
      </div>
    );
  }
}
export default NavBarMain