...
class FirstStep extends React.Component {
  ...
  isValidated() {
    ...
    if(generalData.detalles.dia == null){
      this.setState({
...
class FourthStep extends React.Component {
  ...
  isValidated() {
    ...
    //console.log('Pago', parseFloat(this.state.pago));
    //console.log('Total', parseFloat(this.state.total));
    if(parseFloat(this.state.pago) >= parseFloat(this.state.total)){
      return true;
    }else{
      this.setState({
...
class WizardExample extends React.Component {
  ...
  finishButtonClick(allStates) {
    ...
    //this.updateAsiento(idViaje, asientoElegido);
    //GenerarBoleto();
  }
  ...
  fetchFolio(id){
    fetch(`/api/Usuarios/${id}/id`)
      .then(res => res.json())
      .then(data => {
        generalData.folio = (parseInt(data.noFolio)+1)
        this.setState({
          folio: data.prefijoFolio+"-"+(parseInt(data.noFolio)+1)
        });
      })
      .then(() => {
        this.updateAsiento(generalData.detalles.idViaje, generalData.detalles.noAsiento);
        this.updateFolio(id, generalData.folio);
      });
  }

  updateFolio(id, folio){
    console.log('folio', folio);
    fetch(`/api/Usuarios/${id}/${folio}/noFolio`, {
      method: 'PUT',
  ...
  updateAsiento(id, asiento){
    console.log('Guardando Asiento')
    fetch(`/api/AsignarViajes/${id}/${asiento}`, {
        method: 'PUT',
        //body: JSON.stringify(this.state),
        headers: {
            'Accept': 'application/json',
        ...
        .then(res => {
          //Si el status es OK, procedemos a guardar la venta
          if(res.statusText == 'OK'){
            console.log('se actualizo el asiento');
            venta = {
              'cliente': `${generalData.cliente}`,
              'tipo': `${generalData.clienteTipo}`,
              'razon': `${generalData.razon}`,
              'descuento': `${generalData.descuento.idDescuento}`,
              'origen': `${generalData.origen.id}`,
              'destino': `${generalData.destino.id}`,
              'fecha': `${generalData.detalles.dia}`,
              'anio': `${generalData.detalles.anio}`,
              'diaDelAnio': `${generalData.detalles.diaDelAnio}`,
              'horaSalida': `${generalData.detalles.hora}`,
              'numeroAsiento': `${generalData.detalles.noAsiento}`,
              'statePagado': 'true',
              'activo': 'true',
              'viajado': 'false',
              'idVenta': `${generalData.detalles.idViaje}`,
              'total': `${generalData.cobro.total}`,
              'folio': `${generalData.folio}`,
              'tipoCambio': `${generalData.cambio}`,
              'vendedor': `${generalData.vendedor.id_Usuario}`
            }
          }
        })
        .then(() => {
            ...
            if(venta != null){
              ...
              //lert('datos actualizados');
            }else{
              alert('ha ocurrido un error, porfavor intenta más tarde');
            }
  ...
  saveVenta(){
    fetch('/api/Ventas', { //Aqui hace la peticion al servidor
        method: 'POST',
        ...
        .catch(err => console.error(err)); //No es necesario agregar https://localhost etc, ya que toma el servidor en el que esta corriendo
  }
  ...
  fetchBoleto(){
    this.setState({
      vendedor: generalData.vendedor.nombre+" "+generalData.vendedor.apellidoPaterno+" "+generalData.vendedor.apellidoMaterno,//nombre completo del vendedor
      sucursal: generalData.sucursalVendedor.nombre,//nombre de la sucursal
      pasajero: generalData.nombreCliente,//nombre completo del pasajero
      tipo: generalData.clienteTipo, //tipo de pasajero, si es niño, adulto etc
      origen: generalData.origen.ciudad+" "+generalData.origen.estado+" "+generalData.origen.pais+"\n"+generalData.origen.direccion, //municipio, estado, país y dirección
      fecha: generalData.detalles.dia, //fecha de salida
      hora: generalData.detalles.hora, //hora en formato 00:00
      destino: generalData.destino.ciudad+" "+generalData.destino.estado+" "+generalData.destino.pais+"\n"+generalData.destino.direccion, //municipio, estado, país y dirección
      tipoDeCambio: generalData.cambio, //Si es MX o US
      subtotal: generalData.cobro.subtotal,
      descuento:generalData.cobro.descuento,
      total: generalData.cobro.total,
      noAsiento: generalData.detalles.noAsiento,
