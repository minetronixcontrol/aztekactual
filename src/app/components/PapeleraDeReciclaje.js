...
class FirstStep extends React.Component {
        ...
        if(generalData.origen.id == null || generalData.destino.id == null || generalData.detalles.dia == null ){
            ...
            .then(data => {
                if(data.length > 0){
                    ...
                    alert('No se ha asignado un precio a ese origen y destino');
                    return false;
                }
            })*/
        }
    ...
    getDiaDelAnio(fechaInicial, fechaFinal){
        ...
        //console.log(arr);
        let dia = new Date(arr[2], arr[1], arr[0]);
        //console.log(dia);
        let comienzo = new Date(dia.getFullYear(), 0, 0);
        //console.log(comienzo);
        let dif = dia - comienzo;
        let unDia = 1000 * 60 * 60 * 24;
        dia = Math.ceil(dif / unDia);
        //console.log('dia',dia);
        return dia;*/
    }
...
class ThirdStep extends React.Component {
    ...
    isValidated() {
        ...
        for (let i = 0; i < generalData.noPasajeros; i++) {
            let asiento = document.getElementById(`hiddenSeat${i}`).value;
            let id = '';
            let nombre = '';
            let razon = '';
            let descuento = '';
            ...
            for (const key in this.refs) {
                //console.log(key);
                if(key == `selectCliente${i}`){
                    try {
                        id = this.refs[key].select.select.props.value.value;
                        nombre = this.refs[key].select.select.props.value.name;
                        //console.log('fecha de nacimiento', this.refs[key].select.select.props);
                        typeClient = this.refs[key].select.select.props.value.type;
                    } catch (error) {
                ...
                if(key == `selectRazon${i}`){
                    //console.log(this.refs[key].select.props.value.value);
                    try {
                        razon = this.refs[key].select.props.value.value
                    } catch (error) {
                        alert('Por favor llena todos los campos');
                        return false;
                    }
                ...
                if(key == `selectDescuento${i}`){
                    //console.log(this.refs[key].select.props.value.desc);
                    try {
                        descuento = this.refs[key].select.props.value.desc;
                        //console.log('descuento',this.refs[key].select.props.value);
                        nameDesc = this.refs[key].select.props.value.label;
                        if((nameDesc != 'Ninguno') && (nameDesc != typeClient)){//Aqui validamos que el descuento para niño aplique, si la persona es menor de 10 años, aplica
                            alert('Para el cliente '+nombre+' no aplica el descuento '+nameDesc);
                            return false;
                        }
                    } catch (error) {
                        alert('Por favor llena todos los campos');
    ...
    handleOnChangeCliente(e){
        ...
        //const a = this.refs.selectCliente0.state;
        //console.log(a);
        ...
        let elem = document.getElementsByName('exampleRadios');
        /*for(let i=0; i<elem.length; i++){
            if (elem[i].checked) {
                asiento = elem[i].value;
                pasajeros[i]={
                    'asiento': asiento,
                    'idCliente': e.value,
                    'nombre': e.label
                            ...
                            <div key={'client'+i}>
                                <Row>
                                    <div class="form-check">
                                        <input type="hidden" id={`hiddenSeat${i}`} value={asientos[i]}></input>
                                        {/*<input className="form-check-input" type="radio" name="radioCliente" id={"radioCliente"+asientos[i]} value={asientos[i]} />*/}
                                ...
                                <Row>
                                    ...
                                    <Col lg='4' md='4' sm='12' xs='12'>
                                        <Label for={"selectRazon"+i}>Razón:</Label>
                                        <Select
                                        ref={"selectRazon"+i}
                                        id={"selectRazon"+i}
                                        isSearchable={false}
                                    ...
                                    <Col lg='4' md='4' sm='12' xs='12'>
                                        <Label for={"selectDescuento"+i}>Descuento:</Label>
                                        <Select
                                        ref={"selectDescuento"+i}
                                        id={"selectDescuento"+i}
                                        isSearchable={false}
                    ...
                    .then(data => {
                        if(data.length > 0){
                            if(tipoDeCambio == 'MX'){
                                ...
                                this.setState({
                                    tipoDeCambio: 'MX',
                                    precioDeBoleto: parseFloat(data[0].costo)*generalData.precioDollar,
                                    total: parseFloat(data[0].costo)*generalData.precioDollar
                                });
                                generalData.costoBoleto = parseFloat(data[0].costo)*generalData.precioDollar;
                                generalData.cobro.subtotal = parseFloat(data[0].costo)*generalData.precioDollar;
                                generalData.cobro.total = parseFloat(data[0].costo)*generalData.precioDollar
                            }else{
                        ...
                        if(tipoDeCambio == 'MX'){
                            ...
                            let desc=0;
                            for (let i = 0; i < generalData.noPasajeros; i++) {
                                //console.log(descuentos[i].descuento);
...
export default class papeleraDeReciclaje extends React.Component {
    ...
    finishButtonClick() {
        /*console.log('idUsuario',generalData.vendedor.id_Usuario);
        ...
        .then(data => {
            ...
            for (let i = 0; i < data.rJson.length; i++) {
                datos = {
                    ...
                    'total': parseFloat(generalData.costoBoleto)-parseFloat(descuentos[i].descuento),
    ...
    render() {
        ...
        if(this.state.IrVentaMultiple){return <Redirect to= {"/PapeleraDeReciclaje/"+generalData.vendedor.username}/>};
                    ...
                    <Row>
                        <Col xs={12} md={12} className="mr-auto ml-auto">
                            <ReactWizard
                                steps={steps}
                                navSteps
                                title="VENTA MULTIPLE DE BOLETOS"
                                description=""
                                headerTextCenter
                                validate
                                color="green"
