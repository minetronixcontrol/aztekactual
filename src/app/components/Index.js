...
class FirstStep extends React.Component {
    ...
    componentDidMount(){
        ...
        this.fetchUsuario();
        //Si es acreedor del descuento familiar aparecera el tipo de viaje familiar, en caso contario no se mostrara
    ...
    fetchUsuario(){
        fetch(`/api/Usuarios/${generalData.vendedor.username}`)
            ...
            .then(data => {
                ...
                if(data[0].seguridad == 'Admin' || data[0].seguridad == 'Viajes'){
                    //Buscar los usuarios
    ...
    fetchUsuarios(){
        let optUsuarios = [];
        fetch(`/api/Usuarios/hola/ventaAdmin`)
            .then(res => res.json())
            .then(data => {
                for(let i = 0; i < data.length; i++){
                    if(!(this.state.seguridad === 'Viajes' && (data[i].seguridad === 'Admin' || (data[i].seguridad === 'Viajes' && (data[i]._id !== this.state.idVendedor) )))) {
                        optUsuarios.push({
                            value: data[i]._id,
                ...
                console.log('optUsuarios',optUsuarios);
            })
    }
    ...
    handleChangeFechaHora(e){
        ...
        if(generalData.vendedor.seguridad == 'Admin' || generalData.vendedor.seguridad == 'Viajes'){ //Verificamos que el usuario sea de tipo Admin, si es Admin podrá hacer venta de dias anteriores
            ...
            console.log(`/api/AsignarViajes/${generalData.origen.id}/${generalData.destino.id}/${buscarFecha}`);
    ...
    handleChangeUsuarios(e){
        generalData.vendedor.id_Usuario = e.value;
        generalData.vendedor.username = e.username;
    }
    ...
    render () {
        return (
            <div>
                ...
                <Row>
                    {
                        (this.state.seguridad == 'Admin' || this.state.seguridad == 'Viajes')? 
                        <Col lg={12} md={12} sm={12} xs={12}>
                            <Label for="selectUsuarios">Usuario vendedor:</Label>
                            <div class={(this.state.validationUsuarios) ? 'card border-light': 'card border-danger'}>
                                <Select
                                id="selectUsuarios"
                                defaultValue={{ value: this.state.idVendedor, label: this.state.nombreVendedor}}
                                onChange={this.handleChangeUsuarios}
                                options={this.state.optUsuarios}
                                placeholder = "Selecciona..."
                                />
                            </div>
                            <p class={(this.state.validationUsuarios) ? 'textValid': 'textInvalid'}>Selecciona el vendedor</p>
                        </Col>
                        :
                        ''
                    }
                ...
                <Row form> 
                    ...
                    <Col className="btn-nuevoPasajero" lg={2} md={2} sm={6} xs={6}>
                        <Button onClick={this.handleChangeFechaHora} color="secondary">Comprobar Fecha</Button>
                    </Col>
                    <Col lg={6} md={6} sm={12} xs={12}>                    
    ...
    fetchUsuario(){
        fetch(`/api/Usuarios/${generalData.vendedor.username}`)
            ...
            .then(data => {
                ...
                generalData.vendedor.tipoDescuento = data[0].tipoDescuento;
                this.setState({
                    vendedor: `${generalData.vendedor.nombre} ${generalData.vendedor.apellidoPaterno} ${generalData.vendedor.apellidoMaterno}`
                });
            }).then(() => {
                this.fetchApartados();
            })
            ...
            <div className="text-center">  
                ...
                <Row>
                    ...
                    <div>
                        ...
                        <CardTitle>Seleccione los lugares <br/> 
                            {
                                (generalData.vendedor.seguridad == 'Admin' || generalData.vendedor.seguridad == 'Viajes')?
...
class FourthStep extends React.Component {
    constructor(props){
        ...
        this.handleDescuentoFamiliar = this.handleDescuentoFamiliar.bind(this);
        this.handleChangeCortesia = this.handleChangeCortesia.bind(this);
        this.calcularTotales = this.calcularTotales.bind(this);
        this.fetchDollar = this.fetchDollar.bind(this);

        this.fetchMontoDescuentoFamiliar = this.fetchMontoDescuentoFamiliar.bind(this);

        this.fetchUsuario = this.fetchUsuario.bind(this);
        this.fetchDescuentoCortesia = this.fetchDescuentoCortesia.bind(this);
    ...
    isValidated(){
        ...
        if(this.state.actualizar){
            console.log('actualizar true');
            if(this.state.click < 1){
                if(this.state.pago >= this.state.total){//Si el pago es igual o mayor al total, podemos avanzar
                    this.setState(prevState => ({
            ...
            console.log('actualizar false');
            return false;
        }
    }

    componentDidMount(){
        this.fetchMontoDescuentoFamiliar();
        this.fetchUsuario();
        this.fetchDollar();
        this.fetchDescuentoCortesia();
    }

    fetchDescuentoCortesia(){
        fetch(`/api/DescuentosNuevo/${generalData.vendedor.username}/cortesia`)
            .then(res => res.json())
            .then(data => {
                this.setState({
                    descCortesia: data
    ...
    fetchMontoDescuentoFamiliar(){
        fetch(`/api/DescuentoFamiliar/true/descuentoFamiliar`)
        .then(res => res.json())
        .then(data => {
            if(data.length > 0){
                this.setState({
                    montoDescuentoFamiliar: data[0].dollarDesc
                });
            }
        })
    }
    ...
    handleClickActualizar(e){
        ...
        for (let i = 0; i < generalData.noPasajeros; i++) {
            descuentos[i].descuento = parseFloat(descuentos[i].descuento)
            totalDescuento+= descuentos[i].descuento;
        }
        generalData.descuentoDlls = totalDescuento;
        if(descPacoima){
            alert(`Has seleccionado el tipo de descuento para Pacoima, los boletos tienen un costo de ${dllDescPacoima}`);
            generalData.costoBoleto = dllDescPacoima;
            this.calcularTotales(false, 'US');
        }else{
    ...
    verificarDescFamiliar(cortesia, tipoDeCambio){
            ...
            fetch(`/api/DescuentoFamiliar/${this.state.idSolicitud}`)
                ...
                .then(data => {
                    ...
                    if(data.status == 'true'){
                        ...
                        generalData.descuentoDlls = desc;
                    }else{
                        alert('Tu solicitud aún no ha sido aprovada, comunicate con la matriz');
                    }
                    this.calcularTotales(cortesia, tipoDeCambio);
                })
        }
    }

    calcularTotales(cortesia, tipoDeCambio){
        if(tipoDeCambio == 'MX'){
            console.log('MX');
            generalData.descuentoNeto = parseFloat(generalData.descuentoDlls) * parseFloat(generalData.precioDollar);
            generalData.subtotal = generalData.costoBoleto * parseFloat(generalData.precioDollar) * parseFloat(generalData.noPasajeros);
            generalData.cobro.subtotal = generalData.costoBoleto * parseFloat(generalData.precioDollar) * parseFloat(generalData.noPasajeros);
        }else{
            generalData.descuentoNeto = parseFloat(generalData.descuentoDlls)
            generalData.subtotal = generalData.costoBoleto * parseFloat(generalData.noPasajeros);
            generalData.cobro.subtotal = generalData.costoBoleto * parseFloat(generalData.noPasajeros);
        }
        generalData.total = parseFloat(generalData.subtotal) - parseFloat(generalData.descuentoNeto);
        generalData.cobro.total = parseFloat(generalData.subtotal) - parseFloat(generalData.descuentoNeto);
        if(cortesia){
            generalData.total = generalData.total - parseFloat(cortesia);
            generalData.cobro.total = generalData.total - parseFloat(cortesia);
        }
        this.setState({
            subtotal: generalData.subtotal,
            descuentoNeto: generalData.descuentoNeto,
            total: generalData.total,
            grandTotal: generalData.total
        })
    }
    ...
    handleOnChangeTipoDeCambio(e){
        ...
        if(this.state.tipoDeViaje == 'Familiar'){
            this.verificarDescFamiliar(this.state.cortesia, e.target.value);
        }else{
            this.calcularTotales(this.state.cortesia, e.target.value);
        }
    }

    handleChangeCortesia(e){
        this.setState({
            cortesia: e.target.value
        });
        generalData.cobro.cortesia = e.target.value;
        if(e.target.value != '' && e.target.value != null){
            if(this.state.tipoDeViaje == 'Familiar'){
                this.verificarDescFamiliar(e.target.value, this.state.tipoDeCambio);
            }else{
                this.calcularTotales(e.target.value, this.state.tipoDeCambio);
            }            
        }     
    }
                ...
                <Row>
                    <Col md={{ size: 6}} lg={{ size: 6}} sm={12} xs={12}>
                        ...
                        <tbody>
                            ...
                            </tr>
                            {
                                ...
                                (
                                    <tr>
                                        ...
                                        <td>
                                            <span>{this.state.tipoDeCambio}{' $'}</span>
                                            <Input type="number" min="0" id="inputCortesia" placeholder="0.00" onChange={this.handleChangeCortesia} value={this.state.cortesia}/>
                                        </td>
                                    </tr>
                                )
                                :
                            ...
                            <td>{this.state.tipoDeCambio}{' $'}{this.state.grandTotal}</td>
                            </tr>
                            <tr>
                            <th scope="row">Tipo de cambio</th>
                            <td><CustomInput  type="radio" id="radioUS" name="radioTipoDeCambio" value="US" label="US" onChange={this.handleOnChangeTipoDeCambio} defaultChecked inline />
                                <CustomInput type="radio" id="radioMX" name="radioTipoDeCambio" value="MX" label="MX" onChange={this.handleOnChangeTipoDeCambio} inline /></td>
                            </tr>
                            <tr>
...
export default class papeleraDeReciclaje extends React.Component {
    ...
    finishButtonClick() {
        ...
        console.log(`/api/Usuarios/${generalData.vendedor.id_Usuario}/actualizaYobtenFolio`);
        console.log('pasajeros', pasajeros);
        console.log('razones', razones);
        console.log('descuentos', descuentos);
        console.log('generalData',generalData);
        fetch(`/api/Usuarios/${generalData.vendedor.id_Usuario}/actualizaYobtenFolio`, {
            method: 'PUT',
        ...
        .then(data => {
            ...
            for (let i = 0; i < data.rJson.length; i++) {
                datos = {
                    ...
                    'descuento': (generalData.cobro.cortesia == 0)? parseFloat(descuentos[i].descuento):parseFloat(descuentos[i].descuento)+(parseFloat(generalData.cobro.cortesia)/parseFloat(generalData.noPasajeros)),
                    ...
                    'total': (generalData.cobro.cortesia == 0)? parseFloat(generalData.costoBoleto)-parseFloat(descuentos[i].descuento):parseFloat(generalData.costoBoleto)-parseFloat(descuentos[i].descuento)-(parseFloat(generalData.cobro.cortesia)/parseFloat(generalData.noPasajeros)),
    ...
    render() {
        ...
        if(this.state.IrCorte && (generalData.vendedor.seguridad == 'Admin')){return <Redirect to= {"/Corte/"+generalData.vendedor.usernameV}/>};
        ...
        if(this.state.IrCorte && (generalData.vendedor.seguridad == 'Reservaciones')){return <Redirect to= {"/Reservaciones_Corte/"+generalData.vendedor.username}/>};
        if(this.state.IrCorte && (generalData.vendedor.seguridad == 'Viajes')){return <Redirect to= {"/Viajes_Corte/"+generalData.vendedor.usernameV}/>};
